...
class HitsModelCase(unittest.TestCase):
    ...
    def test_create_hit(self):
        ...
        data = dict(title='Jakiś losowy tytuł',
                    artist_id=12)
        response = self.test_client.post('/api/v1/hits',
    ...
    def test_update_hit(self):
        ...
        response = self.test_client.put('/api/v1/hits/jakis-losowy-tytul',
                                        data=json.dumps(data),
                                        content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = dict(artist_id=17, title='Inny tytuł')
        response = self.test_client.put('/api/v1/hits/rest',
                                        data=json.dumps(data),
                                        content_type='application/json')
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(response.status_code, 204)
        data = dict(artist_id=58)
        response = self.test_client.put('/api/v1/hits/inny-tytul',
                                        data=json.dumps(data),
                                        content_type='application/json')
        self.assertEqual(response.status_code, 204)
